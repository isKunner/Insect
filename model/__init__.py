...
class config:
    def __init__(self, model="MobileNetV2", audio_type="mf-lf-ng", sample="None", epochs=20, batch_size=16, lr=0.001, n_xxcc=90, n_fft=2048, win_length=2048, hop_length=2048, visual_field=True, attention=False):
        """
        ...
        if self.model == "MobileNetV2":
            ...
            self.audio_type = "mf-lf-ng"
            self.init_result()  # 初始化
            self.get_datas = get_datas(audio_type=self.audio_type, sample=self.sample, batch_size=self.batch_size, n_fft=self.n_fft, win_length=self.win_length, hop_length=self.hop_length, n_xxcc=self.n_xxcc)
            self.model_config = Resnet18_config(self.get_datas.train_dl, self.get_datas.val_dl, self.get_datas.train_num, self.get_datas.val_num, visual_field=self.visual_field, attention=self.attention, epochs=self.epochs, lr=self.lr, audio_type=self.audio_type)
    ...
    def validate(self, epoch, running_loss, best_acc):
        ...
        # print(y.size())
        # print(predic.size())
        # print(f"acc_:{np.sum(y.cpu().numpy() == predic.cpu().numpy())}")
        # print(self.model_config.val_num)
