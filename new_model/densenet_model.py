...
# 密集连接块由一个个密集层组成，每个密集层由两层卷积组成
class _DenseLayer(nn.Module):
    def __init__(self,
                 input_c: int,
                 growth_rate: int,  # 增长率，可以理解为通道数
                 bn_size: int,  # 把增长率倍增一下，不理解这里
                 drop_rate: float,
...
class _DenseBlock(nn.ModuleDict):
    ...
    def __init__(self,
        ...
        for i in range(num_layers):
            layer = _DenseLayer(input_c + i * growth_rate, # 因为是全连接层，会把之前的输入都拼接起来
...
class DenseNet(nn.Module):
    ...
    def __init__(self,
                 growth_rate: int = 32,
                 block_config: Tuple[int, int, int, int] = (6, 12, 24, 16), # 每个密集连接块由多少个密集层组成
