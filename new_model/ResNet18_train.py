...
from ResNet18_model import resnet18, resnet34, resnet50


# ResNet18_attention_weight_0.940.pth 是加入了CBAM跑出来的，按照原论文的方式，先通道卷积再空间卷积


class Resnet18_config:
    def __init__(self, train_dl, val_dl, train_num, val_num, seq_length, epochs=20, lr=0.001, visual_field=True, attention=True, audio_type='mfcc'):
        self.net = resnet18(num_classes=2, visual_field=visual_field, attention=attention, audio_type=audio_type, seq_length=seq_length)
        ...
        if visual_field:
            if attention:
                self.save_path = './Resnet18_visual_attention_weight.pth'
            else:
                self.save_path = './Resnet18_visual_weight.pth'
        else:
            if attention:
                self.save_path = './Resnet18_attention_weight.pth'
            else:
                self.save_path = './Resnet18_weight.pth'
...
class config:
    def __init__(self, train_dl, val_dl, train_num, val_num, epochs=20, lr=0.001):
        ...
        self.train_steps = len(train_dl)
        self.model_weight_path = None
        self.save_path = './CNN_LSTM_attention_weight.pth'
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
